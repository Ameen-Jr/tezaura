...
const InteractiveFace = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      meTl.from(".me", { duration: 1, yPercent: 100, ease: "elastic.out(0.5, 0.4)" }, 0.5)
      ...
      const dizzy = gsap.timeline({ paused: true, onComplete: () => { dizzyIsPlaying = false; } });
      dizzy.to(dom.eyes, { duration: 0.01, opacity: 0 }, 0)
           .to(dom.dizzy, { duration: 0.01, opacity: 0.3 }, 0)
           .to(dom.mouth, { duration: 0.01, opacity: 0 }, 0)
           .to(dom.oh, { duration: 0.01, opacity: 0.85 }, 0)
           .to([".head", ".hair-back", ".shadow"], { duration: 6, rotate: 2, transformOrigin: "50% 50%", ease: "elastic.out(1, 0.2)" }, 0)
           .to(dom.me, { duration: 6, rotate: -2, transformOrigin: "50% 100%", ease: "elastic.out(1, 0.2)" }, 0)
           .to(dom.me, { duration: 4, scale: 0.99, transformOrigin: "50% 100%", ease: "elastic.inOut(1, 0.5)" }, 0)
           ...
           .to(dom.eyes, { duration: 0.01, opacity: 1 }, 4)
           .to(dom.dizzy, { duration: 0.01, opacity: 0 }, 4)
           .to(dom.oh, { duration: 0.01, opacity: 0 }, 4)
           .to(dom.mouth, { duration: 0.01, opacity: 1 }, 4);
      ...
      const handleMouseMove = (e) => {
        ...
        const xDiff = e.clientX - faceCenterX;
        const yDiff = e.clientY - faceCenterY;

        // Shake detection
        if (!dizzyIsPlaying && Math.abs(e.movementX) > 50) {
          dizzyIsPlaying = true;
          dizzy.restart();
        }
        if (dizzyIsPlaying) return;

        // Limit range (Clamp) so eyes don't disappear
        const clamp = (num, min, max) => Math.min(Math.max(num, min), max);
        const x = clamp(xDiff / 10, -80, 80); 
        const y = clamp(yDiff / 10, -80, 80);

        const yHigh = y * 0.6; 
        const yLow = y * 0.8;

        gsap.to(dom.face, { yPercent: yLow / 30, xPercent: x / 30, overwrite: "auto" });
        gsap.to(dom.eye, { yPercent: yHigh / 3, xPercent: x / 2, overwrite: "auto" });
        gsap.to(dom.innerFace, { yPercent: y / 6, xPercent: x / 8, overwrite: "auto" });
        gsap.to(dom.hairFront, { yPercent: yHigh / 15, xPercent: x / 22, overwrite: "auto" });
        gsap.to([dom.hairBack, dom.shadow], { yPercent: (yLow / 20) * -1, xPercent: (x / 20) * -1, overwrite: "auto" });
        gsap.to(dom.ear, { yPercent: (y / 1.5) * -1, xPercent: (x / 10) * -1, overwrite: "auto" });
        gsap.to([dom.eyebrowLeft, dom.eyebrowRight], { yPercent: y * 2.5, overwrite: "auto" });
      };

      // Listen to WINDOW for movement everywhere
  ...
  return (
    <div ref={wrapperRef} style={{ width: "100%", height: "100%", display: "flex", justifyContent: "center", alignItems: "center", cursor: "pointer" }}>
      <svg viewBox="0 10 211.73 180" style={{ width: "100%", height: "100%" }}>
        <defs>
        ...
        <g clipPath="url(#background-clip)">
          <g className="me" opacity="0">
            <g className="body">
              <path className="shadow" d="M129.86,48.47s6.76,4.91,10,12.07,7,29.06,11.71,39.82,9.06,22.5,9.91,26.42,1.57,5-2.52,10.2-14.63,12-14.63,12l-11.47,6.8s14.87,9.67,17.68,19.32a71.16,71.16,0,0,1,4.34,18.79l-21.39,4.54L113.2,164.85l-13-11.1L90.31,75.37Z" opacity="0.09" style={{ isolation: "isolate" }} />
