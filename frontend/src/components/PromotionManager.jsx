...
function PromotionManager() {
    ...
    const [resetFees, setResetFees] = useState(false);
    ...
    const handleFinalPromotion = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/promote-year-end`, {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ graduation_year: gradYear, reset_fees: resetFees, reset_marks: resetMarks, reset_attendance: resetAttendance })
            });
            const data = await res.json();
            if (res.ok) {
                ...
                <div style={{ backgroundColor: "white", padding: "30px", borderRadius: "10px", boxShadow: "0 2px 5px rgba(0,0,0,0.1)", textAlign: "center" }}>
                    ...
                    <div style={{ display: "inline-block", textAlign: "left", backgroundColor: "#f8f9fa", padding: "20px", borderRadius: "8px", margin: "20px 0" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                            <div style={{ fontSize: "12px", fontWeight: "bold", color: "#6B7280", textTransform: "uppercase", marginBottom: "4px" }}>
                                Clear old academic year data:
                            </div>

                            <label style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }}>
                                <input type="checkbox" checked={resetFees} onChange={e => setResetFees(e.target.checked)} />
                                <span>🗑️ Delete all fee payment records</span>
                            </label>

                            <label style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }}>
                                <input type="checkbox" checked={resetMarks} onChange={e => setResetMarks(e.target.checked)} />
                                <span>📝 Delete all exam marks & exam definitions</span>
                            </label>

                            <label style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }}>
                                <input type="checkbox" checked={resetAttendance} onChange={e => setResetAttendance(e.target.checked)} />
                                <span>📅 Delete all attendance records</span>
                            </label>

                            {(resetFees || resetMarks || resetAttendance) && (
                                <div style={{ marginTop: "4px", padding: "10px 14px", backgroundColor: "#FEF2F2", border: "1px solid #FCA5A5", borderRadius: "6px", fontSize: "13px", color: "#B91C1C" }}>
                                    ⚠️ Checked items will be <strong>permanently deleted</strong>. Print all reports before proceeding. This cannot be undone.
                                </div>
                            )}
                        </div>
                    </div>
